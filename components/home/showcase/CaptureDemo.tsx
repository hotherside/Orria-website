...
const SPOKEN_TEXT = "I got offered a promotion but it means relocating to another city...";
...
const TIMINGS: Record<Phase, number> = {
  idle: 600,
  recording: 1000,
  dictating: 0, // driven by charIndex
  structuring: 800,
  structured: 3500,
};
...
export function CaptureDemo() {
  ...
  useEffect(() => {
    ...
    switch (phase) {
      case "idle":
        timeoutRef.current = setTimeout(() => setPhase("recording"), TIMINGS.idle);
        break;
      case "recording":
        timeoutRef.current = setTimeout(() => setPhase("dictating"), TIMINGS.recording);
        break;
      case "dictating":
        if (charIndex < SPOKEN_TEXT.length) {
          timeoutRef.current = setTimeout(() => setCharIndex((i) => i + 1), DICTATION_SPEED);
        } else {
          timeoutRef.current = setTimeout(() => setPhase("structuring"), 800);
        }
        break;
      case "structuring":
        timeoutRef.current = setTimeout(() => setPhase("structured"), TIMINGS.structuring);
        break;
      case "structured":
        timeoutRef.current = setTimeout(reset, TIMINGS.structured + 1500);
        break;
    }
          ...
          {phase !== "structured" ? (
            ...
            >
              ...
              <div className="bg-white rounded-xl p-3" style={{ border: "1px solid #E5E7EB" }}>
                ...
                <p className="text-[11px] font-medium" style={{ color: "#111111" }}>Should I take the promotion and relocate?</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <p className="text-[10px]" style={{ color: "#111111" }}>Accept and relocate</p>
                </motion.div>
                <motion.div
                  initial={{ opacity: 0, x: -8 }}
                  animate={{ opacity: 1, x: 0 }}
                ...
                >
                  ...
                  <p className="text-[10px]" style={{ color: "#111111" }}>Stay and negotiate remotely</p>
                </motion.div>
              </div>
              ...
              >
                <span className="px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-600 text-[8px]">Career</span>
